function [LST_result,PET_result,beta_veg_result,qsat_result] = one_row_LST_veg_Feb_to_Dec(row_no,column_max,...
    ...
    Height,z0m_input)
beta_veg_result=repmat(beta_veg,752,column_max);
...
beta_veg_result=repmat(0,752,column_max);

Stefan_Boltzmann_constant=5.67*10^(-8); % W m-2 K-4
surface_emissivity=0.95; % for vegetation
...
a2=0.325; % paramter for heat storage (vegetation)
...
beta_veg_initial=beta_veg_result;
beta_veg_result=beta_veg_initial(751:752,:); % beta for Jan last 2 hours, no precipitation, then equals to 0
...
Albedo=0.119; % albedo of vegetation 0.125 from Elie excel, 0.119 (averaged) from literature
...
for i=3:8042
    ...
    PET_result(isnan(PET_result))=0; % NaN==0
     [m,n]=size(PET_result);
    PET_14days=sum(PET_result(((m-335):m),:),1); % sum of PET during the past 14 days (336hours)
    P_14days=sum(P_input(((406+i):(741+i)),:),1); % sum of precipitation during the past 14 days
    ratio=P_14days./PET_14days; % over the past 14 days, 336 hours
    beta_veg=1+ratio-[0.8+ratio.^oumiga].^(1/oumiga);
    beta_veg_result=real([beta_veg_result;beta_veg]);
    beta_veg_result(isnan(beta_veg_result))=0.1;
    beta_veg_result(beta_veg_result<0)=0.1;
    %% Part 5 Ground heat
    ...
    qa_now=qa(i,:);
    Lamda_now=Lamda(i,:);
    beta_veg_now=beta_veg_result(i,:);
    a1_now=a1(i,:);
    a3_now=a3(i,:);
    ...
    y=@(Ts,SW_net_now,LW_downwards_corrected_now,Ta_now,ra_now,Lamda_now,beta_veg_now,qa_now,a1_now,Qnow_part1,Qnow_1,a3_now)SW_net_now...
        ...
        -rho*Cp.*(Ts-Ta_now)./ra_now-Lamda_now.*beta_veg_now.*rho.*(0.622*0.6108*exp(17.27*(Ts-273.15)./((Ts-273.15)+237.3))./(patm-0.378*0.6108*exp(17.27*(Ts-273.15)./((Ts-273.15)+237.3)))-qa_now)./(ra_now+rc)...
    ...
    for j=1:column_max
        LST(1,j)=fzero(@(Ts)y(Ts,SW_net_now(1,j),LW_downwards_corrected_now(1,j),Ta_now(1,j),ra_now(1,j),Lamda_now(1,j),beta_veg_now(1,j),qa_now(1,j),a1_now(1,j),Qnow_part1(1,j),Qnow_1(1,j),a3_now(1,j)),273); % fzero initial value is assigned the previous hour's LST
    end
