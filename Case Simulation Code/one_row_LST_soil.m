function [LST_result,PET_result] = one_row_LST(row_no,column_max,...
...
a1=0.355; % parameter for heat storage calculation (soil)
a2=0.335; % parameter for heat storage calculation (soil)
a3=-35.3; % parameter for heat storage calculation (soil)
...
Albedo=0.229; % Albedo of soil (0.159 for Elie excel,0.229 from literature)
...
%LW_downwards_corrected=0.9338*LW_downwards+44.081;
LW_downwards_corrected=LW_downwards;
%% Part 3 Sensible heat
z0m=0.1; %% Roughness length related to momentum
...
beta_soil=0; % No precipitation in Jan, beta_soil=0
%%
for i=3:752
    %% Part 5 Ground heat
    ...
    y=@(Ts,SW_net_now,LW_downwards_corrected_now,Ta_now,ra_now,Lamda_now,qa_now,Qnow_part1,Qnow_1)...
        SW_net_now...
        +LW_downwards_corrected_now.*surface_emissivity-surface_emissivity.*Stefan_Boltzmann_constant.*(Ts.^4)...
        -rho*Cp.*(Ts-Ta_now)./ra_now...
        -Lamda_now.*beta_soil.*rho*(0.622*0.6108*exp(17.27*(Ts-273.15)./((Ts-273.15)+237.3))./(patm-0.378*0.6108*exp(17.27*(Ts-273.15)./((Ts-273.15)+237.3)))-qa_now)./ra_now...
    ...
    PET=conversion_factor.*rho.*(qsat_now-qa_now)./(ra_now+rc)./rho_w; % PET of current hour
    PET(PET<0)=0;
