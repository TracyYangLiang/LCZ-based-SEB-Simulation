function [LST_result,PET_result,ra] = one_row_LST_veg(row_no,column_max,...
...
surface_emissivity=0.95; % vegetation
...
a1=0.215; % paramter for heat storage (vegetation)
a2=0.325; % paramter for heat storage (vegetation)
a3=-19.9; % paramter for heat storage (vegetation)
...
PET_first2hours(1,:)=zeros(1,column_max); % PET of the first hour (0am local time), assigned 0
PET_first2hours(2,:)=zeros(1,column_max); % PET of the second hour (1am local time), assigned 0
...
beta_veg=0.1; % No precipitation in Jan, then beta_veg=0.1
%%
for i=3:752
    %% Part 5 Ground heat
    ...
    y=@(Ts,SW_net_now,LW_downwards_corrected_now,Ta_now,ra_now,Lamda_now,qa_now,Qnow_part1,Qnow_1)SW_net_now...
        ...
        -rho*Cp.*(Ts-Ta_now)./ra_now-Lamda_now*beta_veg.*rho*(0.622*0.6108*exp(17.27*(Ts-273.15)./((Ts-273.15)+237.3))./(patm-0.378*0.6108*exp(17.27*(Ts-273.15)./((Ts-273.15)+237.3)))-qa_now)./(ra_now+rc)...
